...
internal static class RewardInjectionPatch
{
    ...
    {
        // 只对大战士 / 小猎人 / 王生效,不污染原版角色。
        if (player.Character is not (BigWarrior or LittleHunter or Wang))
        {
            return;
        }
        ...
        for (int i = 0; i < __result.Count; i++)
        {
            ...
            {
                __result[i] = CostReward(player, hunter, wang);
                __result.Insert(i + 1, EffectReward(player, room.RoomType, hunter, wang));
                if (room.RoomType == RoomType.Elite)
                {
                    __result.Insert(i + 2, EffectReward(player, room.RoomType, hunter, wang));
                }
                return;
            }
        }
    ...
    /// <summary>费用卡奖励按角色分派池:大战士=ironclad 池 / 小猎人=silent 绿池 / 王=regent 橙池。</summary>
    private static CardReward CostReward(Player player, bool hunter, bool wang)
    ...
    private static SpliceReward EffectReward(Player player, RoomType roomType, bool hunter, bool wang)
