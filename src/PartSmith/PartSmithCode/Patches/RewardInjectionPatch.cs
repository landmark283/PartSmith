...
/// <summary>
/// 战斗奖励注入:把"大战士 / 小猎人 / 王 / 骨头人"的战斗卡牌奖励替换成
...
internal static class RewardInjectionPatch
{
    ...
    {
        // 只对大战士 / 小猎人 / 王 / 骨头人生效,不污染原版角色。
        if (player.Character is not (BigWarrior or LittleHunter or Wang or BoneMan))
        {
            return;
        }
        ...
        for (int i = 0; i < __result.Count; i++)
        {
            ...
            {
                __result[i] = CostReward(player, hunter, wang, boneMan);
                __result.Insert(i + 1, EffectReward(player, room.RoomType, hunter, wang, boneMan));
                if (room.RoomType == RoomType.Elite)
                {
                    __result.Insert(i + 2, EffectReward(player, room.RoomType, hunter, wang, boneMan));
                }
                return;
            }
        }
    ...
    /// <summary>费用卡奖励按角色分派池:大战士=ironclad 池 / 小猎人=silent 绿池 / 王=regent 橙池 / 骨头人=necrobinder 粉池。</summary>
    private static CardReward CostReward(Player player, bool hunter, bool wang, bool boneMan)
    ...
    private static SpliceReward EffectReward(Player player, RoomType roomType, bool hunter, bool wang, bool boneMan)
