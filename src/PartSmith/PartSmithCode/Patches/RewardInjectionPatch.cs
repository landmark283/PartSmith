...
internal static class RewardInjectionPatch
{
    private static void Postfix(List<Reward> __result, Player player, AbstractRoom room)
    {
        ...
        for (int i = 0; i < __result.Count; i++)
        {
            ...
            {
                __result[i] = hunter
                    ? PartSmithRewardFactory.CreateHunterCostCardReward(player)
                    : PartSmithRewardFactory.CreateCostCardReward(player);
                __result.Insert(i + 1, hunter
                    ? PartSmithRewardFactory.CreateHunterEffectCardReward(player, room.RoomType)
                    : PartSmithRewardFactory.CreateEffectCardReward(player, room.RoomType));
                if (room.RoomType == RoomType.Elite)
                {
                    __result.Insert(i + 2, hunter
                        ? PartSmithRewardFactory.CreateHunterEffectCardReward(player, room.RoomType)
                        : PartSmithRewardFactory.CreateEffectCardReward(player, room.RoomType));
                }
                return;
